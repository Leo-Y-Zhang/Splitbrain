...
| C03 | `internal/checker/checker.go` | A cached configuration is the placed set AND the model state. Keying on the placed set alone abandons orderings the search never explored, and the verdict stops being exhaustive. | `TestAdversarialDifferentialAgainstBruteForce (+3 more)` |
