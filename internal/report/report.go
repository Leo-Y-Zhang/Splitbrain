...
type pageData struct {
	...
	Drawn    int
	Total    int
	// Rewritten counts the drawn operations whose result the drawing does not
	// show, because they were still in flight where the history was cut.
	Rewritten int
	Truncated bool
	Note      string
	Key       string
...
func (in Input) build() (pageData, error) {
	...
	note := "the minimal failing truncation: every operation up to the moment the violation became unavoidable, " +
		"with anything still in flight at that moment drawn as unanswered, because that is all the client knew then"
	if len(shown) == 0 {
		if key != "" {
	...
	truncated := len(in.Verdict.Ops) > 0
	...
	for i, op := range shown {
		...
		if truncated && op.Outcome == history.Info {
			d.Rewritten++
		}
	...
	d.Stats = []stat{
		...
		{"operations drawn", fmt.Sprint(d.Drawn)},
...
<main>
  ...
  learned the answer; a dashed bar reaching the right edge had not been answered
  {{if .Truncated}}by the point this drawing stops{{else}}at all{{end}}.
  {{if .Truncated}}The counts above are for the whole recorded run; the drawing is the truncation,
  {{if .Rewritten}}and {{.Rewritten}} of the operations in it were still in flight where it was cut, so their
  results are deliberately not shown.{{else}}so an operation may appear here without the result the run
  eventually recorded for it.{{end}}{{end}}</footer>
</main>
</body>
</html>
`
